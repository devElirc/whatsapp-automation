...
(async () => {
  ...
  try {
    ...
    await page.waitForSelector('role=button[name="Entrar com número de telefone"]', { timeout: 30000 });
    await page.click('role=button[name="Entrar com número de telefone"]');


    // Wait for and fill phone number input
    await page.waitForSelector('input[aria-label="Insira seu número de telefone."]', { timeout: 10000 });
    await page.fill('input[aria-label="Insira seu número de telefone."]', phoneNumber.replace('+', ''));

    // Click "Avançar"
    await page.waitForSelector('button:has-text("Avançar")', { timeout: 10000, state: 'visible' });
    await page.click('button:has-text("Avançar")');

    // Wait for PIN with improved retry logic and debugging
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        await page.waitForSelector('[aria-details="link-device-phone-number-code-screen-instructions"]', { timeout: 30000 });
